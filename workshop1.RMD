...
Ciriello et al, 2015. 
  ...
  * breast cancer is the perfect dataset to study because there is enough that is known so that we can check our results, however there is still a lot more unknown for some interesting mining.
  * we will start with first principle by studying public data and see what we can find that matches with what is known.
  * we will pretend we are the only people in the world with this dataset and there are 2 goals.
    + "discover" a general molecular profile for this dataset 
    + study the TCGA sample you were given.
    + https://pubmed.ncbi.nlm.nih.gov/26451490/

### the goal really is to get you excited about bioinformatics and show you first hand how how much we can get just from a couple of hours of digging around can yield very relevant results. Have fun and feel free to reach out for questions.
  + Alex: alex.lee2@ucsf.edu
  + Henry: henry.martell@ucsf.edu

### we assume that you are at least familiar with R at the basic level, if not is fine as well, and we will guide you as needed. 
...
* This portal is hierarchically structured. This is the workflow will use. 
...
# qqq how do you think we can use this to look for the dataset we need among > 200 studies?
...
## an index is position in a vector, eg apple, pear, peach, 
...
* Important here you need to repalced this with yourown assigned TCGA ID. Let us know immediately if your assigned TCGA is not in the list.  
...
* Lets take a look at some of our favorite genes. 
...
# qqq based on the lecture previously can you figure out what each of these mean?  
...
# there may be an error in th latest cbio data set, the impact score is non longer present here. 

mt [   mt$functional_impact_score == "H", ]
table (  mt$functional_impact_score)


# qqq if you have more than one sample, say you want to check all 10 samples in the class, how would you do this? 
...
# X go to https://cancer.sanger.ac.uk/cosmic and compare this. 
...
# qqq by looking at this plot and quantile we can see that there is a good amount of mutations that are under 21 
...
imp
# qqq how does this compare to our plot above 
